...
public class BpmnDeploymentTest extends PluggableActivitiTestCase {
  ...
  public void testDeployTwoProcessesWithDuplicateIdAtTheSameTime() {
	try {
		...
		repositoryService.createDeployment().enableDuplicateFiltering().addClasspathResource(bpmnResourceName)
				.addClasspathResource(bpmnResourceName2).name("duplicateAtTheSameTime").deploy();
		fail();
	} catch (Exception e) {
