...
public class SimpleSecurityServiceManagerMockTest {
	...
	private void testHandlingRunAs(RunAs incomingRunAs, RunAs outgoingRunAs) {
		...
		if (outgoingRunAs != null) {
			Assert.assertEquals("RunAs identity has to be same as previous outgoing RunAs.", outgoingRunAs,
					result.getIncomingRunAs());
		} else if (incomingRunAs != null) {
			Assert.assertEquals("RunAs identity has to be same as previous incoming RunAs.", incomingRunAs,
					result.getIncomingRunAs());
		} else {
