...
public class TaskQueryTest extends PluggableActivitiTestCase {
  ...
  public void testQueryByAssigneeIdsOr() {
		TaskQuery query = taskService.createTaskQuery().or().taskId("invalid")
				.taskAssigneeIds(Arrays.asList(GONZO, KERMIT));
		...
		if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.AUDIT)) {
			assertEquals(1, historyService.createHistoricTaskInstanceQuery().or().taskId("invalid")
					.taskAssigneeIds(Arrays.asList(GONZO, KERMIT)).count());
			assertEquals(0, historyService.createHistoricTaskInstanceQuery().or().taskId("invalid")
					.taskAssigneeIds(Arrays.asList(KERMIT, "kermit2")).count());
		}
		...
		query = taskService.createTaskQuery().or().taskId("invalid")
				.taskAssigneeIds(Arrays.asList(GONZO, "testAssignee"));
		assertEquals(2, query.count());
		assertEquals(2, query.list().size());
		if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.AUDIT)) {
			assertEquals(2, historyService.createHistoricTaskInstanceQuery().or().taskId("invalid")
					.taskAssigneeIds(Arrays.asList(GONZO, "testAssignee")).count());
		}
